...
/**
 * 修改了一部分代码，让它更好记：与avlTreeMap相似度更高，独立的部分在不更改时间复杂度的情况下更好些
 *
 * 包含：
 *  maintain(SBTNode cur), delete(SBTNode cur, K key), getIndex(int kth(start from 1))
 *
 * 其余不一样的方法都是在private methods： add, delete方法都涉及到了size++或者size--
 *
 * BST的public相关计算都是跟AVLTreeMap一模一样的。
 *
 */
...
//					cur = des;
                    ...
                    cur.r = delete(cur.r, des.key); // cur.r -> des 沿途(inclusive)的每个节点都执行 size--
                    des.l = cur.l;
                    des.r = cur.r;
                    // TODO: 【错误-遗漏】后继(/前驱)节点替代被删除节点时，要更新平衡因子 (size)
                    cur = des;
                }
            }
            // TODO: 【错误-遗漏】一定要在最后把新替代cur，重新计算平衡因子 (因为在 key==cur.key && 左右孩子存在，后继节点替代cur，整个新cur的左右子树发生了改变，必须重新计算size)
            if (cur != null) {
...
//            cur = maintain(cur);
        ...
        // TODO: 这个方法比较重要，可以动态求第kth个数字。   唯一注意的是 kth范围 [1, n]
        private SBTNode<K, V> getIndex(SBTNode<K, V> cur, int kth) {
            // 这个方法完全可以单独一个变量 int leftAndMiddle = 左子树.size + 1;
            if (kth == (cur.l != null ? cur.l.size : 0) + 1) {  // kth == leftAndMiddle
                return cur;
            } else if (kth <= (cur.l != null ? cur.l.size : 0)) { // kth < leftAndMiddle
                return getIndex(cur.l, kth);
            } else {
                return getIndex(cur.r, kth - (cur.l != null ? cur.l.size : 0) - 1);  // getIndex(cur.r, kth - leftAndMiddle)
            }
        }
        ...
        public K getIndexKey(int index) {
            ...
            // TODO: 这个地方要注意，传入的index范围是[0,?]， 而我们的内部方法getIndex（int kth）是要index+1的 ，因为范围是[1,?]
