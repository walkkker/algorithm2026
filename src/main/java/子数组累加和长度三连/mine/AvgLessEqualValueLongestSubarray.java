...
public class AvgLessEqualValueLongestSubarray {
    ...
    public static int waysMine(int[] arr, int v) {
        // <= 平均值  ===>  <= 平均值0 ===>  <= 累加和0  ===> 子数组累加和长度问题第三连
        int n = arr.length;
        for (int i = 0; i < n; i++) {
            arr[i] = arr[i] - v;
        }
        ...
        for (int L = 0; L < n; L++) {
            ...
            maxLen = Math.max(maxLen, end - L);    // [L, end)
            if (end > L) {
                sum -= arr[L];
            } else {
                end = end + 1;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testTime; i++) {
            ...
            if (ans1 != ans2 || ans1 != ans3 || ans1 != ans4) {
                ...
                System.out.println("我的方法得到的最大长度：" + ans4);
                System.out.println("=========================");
            }
        }
