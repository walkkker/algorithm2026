...
public class Code03_BestArrange {
    ...
    public static int process(Program[] programs, int done, int timeLine) {
        ...
        for (int i = 0; i < programs.length; i++) {
            ...
            // 不加else的另一个解释，我们的递归 就是考虑 【所有安排上的排列情况，哪一个最好】。
            // 所以，枚举递归时，只考虑 当前可以安排上的会议。 无法安排的会议不考虑（直接剪枝）。


            // 加上这个else条件是对的，但是会导致时间复杂度来到完整的O(n!)，导致跑的特别慢
            // 而去掉这个else条件是合理的，我们去跑else的情况，实际会被if的后续情况覆盖。 没有必要去跑一个不可行i的结果。
//            else {
//                Program[] next = copyButExcept(programs, i);
//                max = Math.max(max, process(next, done, timeLine));
//            }
        }
        return max;
    }
